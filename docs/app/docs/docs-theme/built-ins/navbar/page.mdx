...
在导航栏上显示一个跳转到项目主页的按钮。默认情况下，该按钮会链接到 Nextra 的 GitHub 仓库。

你可以通过配置 `projectLink` 和 `projectIcon` 来自定义项目链接，例如将其指向你的 GitLab 仓库：

![项目链接](/assets/docs/project-link.png)
...
在导航栏上显示一个跳转到项目论坛或其他社交媒体的按钮。

你可以通过配置 `chatLink` 和 `chatIcon` 来自定义聊天链接，例如将其指向你的 Twitter 账号：
...
查看[页面配置](/docs/docs-theme/page-configuration#navbar-items)了解如何向导航栏添加自定义菜单或链接。
